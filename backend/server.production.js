...
app.post("/api/ad-bookings/:bookingId/create-qr",bookingUploadLimiter,async(req,res,next)=>{try{
 if(!razorpay)return res.status(503).json({message:"Razorpay payment is not configured"});
 const bookingId=boundedText(req.params.bookingId,80),b=await AdBooking.findOne({bookingId});
 if(!b)return res.status(404).json({message:"Booking नहीं मिली"});
 if(b.paymentStatus==="paid")return res.json({paid:true,booking:{bookingId:b.bookingId,amount:b.amount,status:b.status,paymentStatus:b.paymentStatus}});
 if(b.razorpayQrId){
  try{
   const existing=await razorpay.qrCode.fetch(b.razorpayQrId);
   if(existing?.status==="active"&&existing?.image_url)return res.json({qrId:existing.id,imageUrl:existing.image_url,amount:existing.payment_amount,currency:"INR",bookingId:b.bookingId});
  }catch{}
 }
 const paymentAmount=Math.round(Number(b.amount)*100);
 if(!Number.isFinite(paymentAmount)||paymentAmount<100)return res.status(400).json({message:"Invalid Razorpay payment amount"});
 const qr=await razorpay.qrCode.create({
  type:"upi_qr",
  name:"Awaaz Rajasthan Ad "+b.bookingId,
  usage:"single_use",
  fixed_amount:true,
  payment_amount:paymentAmount,
  description:"Ad Fees "+b.bookingId,
  close_by:Math.floor(Date.now()/1000)+24*60*60,
  notes:{bookingId:b.bookingId,businessName:b.businessName,position:b.position}
 });
 b.razorpayQrId=String(qr.id||"");
 b.razorpayQrImageUrl=String(qr.image_url||"");
 b.paymentMethod="upi";
 b.paymentStatus="pending";
 await b.save();
 if(!b.razorpayQrId||!b.razorpayQrImageUrl)return res.status(502).json({message:"Razorpay QR create नहीं हुआ"});
 res.json({qrId:b.razorpayQrId,imageUrl:b.razorpayQrImageUrl,amount:paymentAmount,currency:"INR",bookingId:b.bookingId});
}catch(e){next(e)}});

app.get("/api/ad-bookings/:bookingId/verify-qr",bookingUploadLimiter,async(req,res,next)=>{try{
 if(!razorpay)return res.status(503).json({message:"Razorpay payment is not configured"});
 const bookingId=boundedText(req.params.bookingId,80),b=await AdBooking.findOne({bookingId});
 if(!b)return res.status(404).json({message:"Booking नहीं मिली"});
 if(b.paymentStatus==="paid")return res.json({paid:true,booking:{bookingId:b.bookingId,amount:b.amount,paymentStatus:b.paymentStatus,status:b.status}});
 if(!b.razorpayQrId)return res.json({paid:false,booking:{bookingId:b.bookingId,amount:b.amount,paymentStatus:b.paymentStatus,status:b.status}});
 const payments=await razorpay.qrCode.fetchAllPayments(b.razorpayQrId,{count:10});
 const expectedAmount=Math.round(Number(b.amount)*100);
 const paid=(payments?.items||[]).find(p=>String(p.status)==="captured"&&Number(p.amount)===expectedAmount&&String(p.currency)==="INR");
 if(!paid)return res.json({paid:false,booking:{bookingId:b.bookingId,amount:b.amount,paymentStatus:b.paymentStatus,status:b.status}});
 b.razorpayPaymentId=String(paid.id||"");
 b.paymentTxnId=String(paid.id||"");
 b.paymentStatus="paid";
 b.status="paid";
 await b.save();
 res.json({paid:true,booking:{bookingId:b.bookingId,amount:b.amount,paymentStatus:b.paymentStatus,status:b.status},paymentId:b.razorpayPaymentId});
}catch(e){next(e)}});
