...
});await Promise.all([News,Ad,AdBooking,Admin,Subscriber,Epaper,Category,HomeNavButton,LegalSettings,Grievance,AdPrice,AdminOtp].map(model=>model.createIndexes()));await Promise.all(DEFAULT_CATEGORY_ROWS.map(([name,icon,sortOrder])=>Category.updateOne({name},{$setOnInsert:{name,icon,sortOrder,active:true}},{upsert:true})));
