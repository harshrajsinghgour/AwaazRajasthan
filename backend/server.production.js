...
const categorySchema=new mongoose.Schema({name:{type:String,required:true,unique:true,trim:true,maxlength:60},icon:{type:String,default:"📰",maxlength:8},active:{type:Boolean,default:true,index:true},sortOrder:{type:Number,default:0}},{timestamps:true});const homeNavButtonSchema=new mongoose.Schema({label:{type:String,required:true,trim:true,maxlength:40},icon:{type:String,default:"📰",trim:true,maxlength:8},action:{type:String,enum:["latest","breaking","trending","video","photo","category"],default:"latest",index:true},category:{type:String,default:"",trim:true,maxlength:60},sortOrder:{type:Number,default:0,index:true},active:{type:Boolean,default:true,index:true}},{timestamps:true});
...
const News=mongoose.model("News",newsSchema),Ad=mongoose.model("Ad",adSchema),AdBooking=mongoose.model("AdBooking",adBookingSchema),Admin=mongoose.model("Admin",adminSchema),Subscriber=mongoose.model("Subscriber",subscriberSchema),Epaper=mongoose.model("Epaper",epaperSchema),Category=mongoose.model("Category",categorySchema),HomeNavButton=mongoose.model("HomeNavButton",homeNavButtonSchema),AdPrice=mongoose.model("AdPrice",adPriceSchema),AdminOtp=mongoose.model("AdminOtp",adminOtpSchema);
...
app.get("/api/home-buttons",async(_r,res,next)=>{try{const rows=await HomeNavButton.find({active:true}).sort({sortOrder:1,_id:1}).lean();res.json({buttons:rows});}catch(e){next(e);}});
