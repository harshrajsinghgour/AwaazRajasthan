...
app.get("/api/legal-settings",async(_r,res,next)=>{try{const x=await LegalSettings.findOne({key:"default"}).lean();res.json({settings:x||{grievanceOfficerName:"",grievanceOfficerEmail:"",grievanceAddress:"",privacyEmail:""}});}catch(e){next(e);}});
...
async function bootstrap(){if(!process.env.MONGODB_URI){if(PROD)throw new Error("MONGODB_URI is required in production");return;}if(PROD&&!B2_ENABLED)throw new Error("B2 durable media storage is required in production");await mongoose.connect(process.env.MONGODB_URI);await Promise.all([News,Ad,AdBooking,Admin,Subscriber,Epaper,Category,HomeNavButton,LegalSettings,Grievance,AdPrice,AdminOtp].map(model=>model.createIndexes()));await Promise.all(DEFAULT_CATEGORY_ROWS.map(([name,icon,sortOrder])=>Category.updateOne({name},{$setOnInsert:{name,icon,sortOrder,active:true}},{upsert:true})));
