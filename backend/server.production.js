...
app.use(compression());
app.get("/api/health",(req,res)=>{res.set("Cache-Control","no-store");res.status(200).json({ok:true,service:"awaaz-rajasthan-api",time:new Date().toISOString()});});
...
const joinApplicationLimiter=rateLimit({windowMs:15*60*1000,limit:20,standardHeaders:"draft-8",legacyHeaders:false,message:{message:"बहुत अधिक applications भेजे गए हैं। थोड़ी देर बाद फिर कोशिश करें।"}});
...
const AD_POSITIONS=["header_strip","home_top","after_tools","home_inline","latest_inline","sidebar","article_top","article_bottom","district_bottom","footer_top","mobile","desktop"];
const AD_DEVICES=["all","mobile","desktop"];
const AD_BOOKING_RATES={header_strip:1400,home_top:1200,after_tools:1000,home_inline:900,latest_inline:800,sidebar:700,article_top:900,article_bottom:700,district_bottom:700,footer_top:600,mobile:800,desktop:1000};
const DEFAULT_AD_PRICE_META={header_strip:{label:"Header Strip",description:"Category menu के नीचे"},home_top:{label:"Home Top",description:"मुख्य खबरों के सबसे ऊपर"},after_tools:{label:"After Tools",description:"Quick tools के बाद"},home_inline:{label:"Home Middle",description:"Hero के बाद"},latest_inline:{label:"Latest Inline",description:"Latest feed के नीचे"},sidebar:{label:"Sidebar",description:"Desktop side panel"},article_top:{label:"Article Top",description:"खबर के ऊपर"},article_bottom:{label:"Article Bottom",description:"खबर के नीचे"},district_bottom:{label:"District Bottom",description:"जिलेवार section के नीचे"},footer_top:{label:"Footer Top",description:"Footer के ऊपर"},mobile:{label:"Mobile",description:"Mobile-specific placement"},desktop:{label:"Desktop",description:"Desktop-specific placement"}};
const AD_BOOKING_POSITION_LABELS={header_strip:"Header Strip",home_top:"Home Top",after_tools:"After Tools",home_inline:"Home Middle",latest_inline:"Latest Inline",sidebar:"Sidebar",article_top:"Article Top",article_bottom:"Article Bottom",district_bottom:"District Bottom",footer_top:"Footer Top",mobile:"Mobile",desktop:"Desktop"};
...
const grievanceSchema=new mongoose.Schema({ticketId:{type:String,unique:true,index:true},name:{type:String,required:true,trim:true,maxlength:120},email:{type:String,required:true,trim:true,maxlength:180},mobile:{type:String,default:"",trim:true,maxlength:20},subject:{type:String,required:true,trim:true,maxlength:200},message:{type:String,required:true,trim:true,maxlength:5000},status:{type:String,enum:["received","under_review","resolved","rejected"],default:"received",index:true},adminNote:{type:String,default:"",trim:true,maxlength:2000},createdAt:{type:Date,default:Date.now},updatedAt:{type:Date,default:Date.now}});
 const joinApplicationSchema=new mongoose.Schema({applicationId:{type:String,unique:true,index:true},name:{type:String,required:true,trim:true,maxlength:120},email:{type:String,required:true,lowercase:true,trim:true,maxlength:180},mobile:{type:String,required:true,trim:true,maxlength:20},city:{type:String,default:"",trim:true,maxlength:100},district:{type:String,default:"",trim:true,maxlength:80,index:true},role:{type:String,required:true,trim:true,maxlength:100},experience:{type:String,default:"",trim:true,maxlength:1200},portfolio:{type:String,default:"",trim:true,maxlength:500},message:{type:String,default:"",trim:true,maxlength:2500},status:{type:String,enum:["new","reviewing","shortlisted","accepted","rejected"],default:"new",index:true},consentAt:{type:Date,default:null},adminNote:{type:String,default:"",trim:true,maxlength:2000},createdAt:{type:Date,default:Date.now,index:true},updatedAt:{type:Date,default:Date.now}});
...
const newsRevisionSchema=new mongoose.Schema({
 ...
 source:{type:String,enum:["create","edit","autosave","restore","schedule"],default:"edit",index:true},
 savedBy:{type:mongoose.Schema.Types.ObjectId,ref:"Admin",default:null},
 snapshot:{type:mongoose.Schema.Types.Mixed,required:true},
 createdAt:{type:Date,default:Date.now,index:true}
},{timestamps:false});
newsRevisionSchema.index({newsId:1,version:-1});
...
const News=mongoose.model("News",newsSchema),NewsRevision=mongoose.model("NewsRevision",newsRevisionSchema),Ad=mongoose.model("Ad",adSchema),AdBooking=mongoose.model("AdBooking",adBookingSchema),Admin=mongoose.model("Admin",adminSchema),Subscriber=mongoose.model("Subscriber",subscriberSchema),Epaper=mongoose.model("Epaper",epaperSchema),Category=mongoose.model("Category",categorySchema),HomeNavButton=mongoose.model("HomeNavButton",homeNavButtonSchema),LegalSettings=mongoose.model("LegalSettings",legalSettingsSchema),Grievance=mongoose.model("Grievance",grievanceSchema),JoinApplication=mongoose.model("JoinApplication",joinApplicationSchema),AdPrice=mongoose.model("AdPrice",adPriceSchema),AdminOtp=mongoose.model("AdminOtp",adminOtpSchema);
...
async function saveNewsRevision(news,admin,source="edit"){
 try{
  const latest=await NewsRevision.findOne({newsId:news._id}).sort({version:-1}).select("version").lean();
  const version=Number(latest?.version||0)+1;
  const snapshot=news.toObject?news.toObject():{...news};
  delete snapshot._id;
  await NewsRevision.create({newsId:news._id,version,source,savedBy:admin?._id||null,snapshot});
  return version;
 }catch(error){console.warn("News revision save failed:",error?.message||error);return null;}
}
const NEWS_PUBLIC_FILTER={status:"published",$or:[{publishedAt:null},{publishedAt:{$lte:new Date()}}]};
...
app.get("/api/rss.xml",async(_req,res,next)=>{try{
 const rows=await News.find({status:"published",publishedAt:{$lte:new Date()}}).select("slug title excerpt content publishedAt updatedAt category").sort({publishedAt:-1,createdAt:-1}).limit(50).lean();
 const esc=v=>String(v??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&apos;");
 const base=String(process.env.PUBLIC_FRONTEND_URL||process.env.FRONTEND_URL||"https://awaazrajasthan.vercel.app").split(",")[0].trim().replace(/\/$/,"");
...
app.get("/api/news",async(req,res,next)=>{try{
 ...
 if(!Array.isArray(items)){
   const f={status:"published",publishedAt:{$lte:new Date()}};if(category&&category!=="होम")f.category=category;if(location)f.location=location;if(q)f.$text={$search:q};
...
app.get("/api/news/:id/preview",async(req,res,next)=>{try{const key=boundedText(req.params.id,180);const item=mongoose.isValidObjectId(key)?await News.findOne({_id:key,status:"published",publishedAt:{$lte:new Date()}}).lean():await News.findOne({$or:[{slug:key},{shareCode:key}],status:"published",publishedAt:{$lte:new Date()}}).lean();if(!item)return res.status(404).json({message:"News not found"});res.set("Cache-Control","public, max-age=60, s-maxage=300, stale-while-revalidate=900");res.json({news:item,data:item});}catch(e){next(e);}});
app.get("/api/news/:id",async(req,res,next)=>{try{const key=boundedText(req.params.id,180);const query=mongoose.isValidObjectId(key)?{_id:key,status:"published",publishedAt:{$lte:new Date()}}:{ $or:[{slug:key},{shareCode:key}],status:"published",publishedAt:{$lte:new Date()}};const item=await News.findOneAndUpdate(query,{$inc:{views:1}},{new:true}).lean();if(!item)return res.status(404).json({message:"News not found"});res.json({news:item,data:item});}catch(e){next(e);}});
...
app.post("/api/grievances",async(req,res,next)=>{try{const name=boundedText(req.body?.name,120),email=String(req.body?.email||"").trim().toLowerCase(),mobile=boundedText(req.body?.mobile,20),subject=boundedText(req.body?.subject,200),message=boundedText(req.body?.message,5000);if(!name||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||!subject||!message)return res.status(400).json({message:"नाम, सही ईमेल, विषय और शिकायत संदेश आवश्यक हैं।"});const ticketId="AR-GR-"+Date.now().toString(36).toUpperCase()+"-"+crypto.randomBytes(3).toString("hex").toUpperCase();const g=await Grievance.create({ticketId,name,email,mobile,subject,message});res.status(201).json({ok:true,ticketId:g.ticketId,message:"शिकायत दर्ज हो गई है।"});}catch(e){next(e);}});
 app.post("/api/join-applications",joinApplicationLimiter,async(req,res,next)=>{try{const name=boundedText(req.body?.name,120),email=String(req.body?.email||"").trim().toLowerCase(),mobile=boundedText(req.body?.mobile,20),city=boundedText(req.body?.city,100),district=boundedText(req.body?.district,80),role=boundedText(req.body?.role,100),experience=boundedText(req.body?.experience,1200),portfolio=String(req.body?.portfolio||"").trim().slice(0,500),message=boundedText(req.body?.message,2500),consent=Boolean(req.body?.consent),website=String(req.body?.website||"").trim();if(website)return res.status(400).json({message:"आवेदन सत्यापन विफल हुआ।"});if(!name||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||!mobile||!role||!consent)return res.status(400).json({message:"नाम, सही ईमेल, मोबाइल, भूमिका और सहमति आवश्यक हैं।"});if(portfolio&&!/^https?:\/\//i.test(portfolio))return res.status(400).json({message:"Portfolio / profile link सही URL होना चाहिए।"});const applicationId="AR-JOIN-"+new Date().toISOString().slice(0,10).replace(/-/g,"")+"-"+crypto.randomBytes(3).toString("hex").toUpperCase();const item=await JoinApplication.create({applicationId,name,email,mobile,city,district,role,experience,portfolio,message,consentAt:new Date()});res.status(201).json({ok:true,applicationId:item.applicationId,message:"आपका Join Awaaz Rajasthan आवेदन सफलतापूर्वक भेज दिया गया है।"});}catch(e){next(e);}});
...
app.get("/api/admin/news/:id/revisions",auth,permissions("news:read"),async(req,res,next)=>{try{
 const item=await News.findById(req.params.id).select("category location").lean();if(!item)return res.status(404).json({message:"News not found"});
 if(!adminCanPost(req.admin,item.category,item.location))return res.status(403).json({message:"Permission denied"});
 const revisions=await NewsRevision.find({newsId:item._id}).sort({version:-1}).limit(100).populate("savedBy","name email").lean();
 res.json({revisions});
}catch(e){next(e)}});
app.post("/api/admin/news/:id/autosave",auth,permissions("news:write"),async(req,res,next)=>{try{
 const item=await News.findById(req.params.id);if(!item)return res.status(404).json({message:"News not found"});
 if(!adminCanPost(req.admin,req.body?.category||item.category,req.body?.location||item.location))return res.status(403).json({message:"Permission denied"});
 const snapshot={...item.toObject()};
 for(const key of ["title","slug","excerpt","content","category","location","image","video","author","featured","latest","breaking","publishedAt"]){
  if(req.body?.[key]!==undefined)snapshot[key]=req.body[key];
 }
 const latest=await NewsRevision.findOne({newsId:item._id}).sort({version:-1}).select("version").lean();
 const version=Number(latest?.version||0)+1;
 const revision=await NewsRevision.create({newsId:item._id,version,source:"autosave",savedBy:req.admin._id,snapshot});
 res.status(201).json({ok:true,revisionId:revision._id,version});
}catch(e){next(e)}});
app.post("/api/admin/news/:id/revisions/:revisionId/restore",auth,permissions("news:write"),async(req,res,next)=>{try{
 const item=await News.findById(req.params.id);const revision=await NewsRevision.findOne({_id:req.params.revisionId,newsId:req.params.id}).lean();
 if(!item||!revision)return res.status(404).json({message:"Revision not found"});
 if(!adminCanPost(req.admin,item.category,item.location))return res.status(403).json({message:"Permission denied"});
 const s=revision.snapshot||{};const allowed=["title","slug","excerpt","content","category","location","image","video","author","status","featured","latest","breaking","publishedAt"];
 for(const key of allowed)if(s[key]!==undefined)item[key]=s[key];
 if(item.status==="published"&&!item.publishedAt)item.publishedAt=new Date();
 await item.save();await saveNewsRevision(item,req.admin,"restore");
 if(item.status==="published")meiliIndexDocuments([item]).catch(()=>{});
 broadcastNewsEvent("news-updated",{id:String(item._id),slug:item.slug,action:"restored"});
 res.json({news:item});
}catch(e){next(e)}});
...
app.get("/api/admin/grievances",auth,ownerOnly,async(_r,res,next)=>{try{const items=await Grievance.find({}).sort({createdAt:-1}).limit(500).lean();res.json({grievances:items});}catch(e){next(e)}});
 app.get("/api/admin/join-applications",auth,ownerOnly,async(req,res,next)=>{try{const status=String(req.query?.status||"").trim();const filter=status&&["new","reviewing","shortlisted","accepted","rejected"].includes(status)?{status}:{};const items=await JoinApplication.find(filter).sort({createdAt:-1}).limit(500).lean();res.json({applications:items,data:items});}catch(e){next(e)}});
 app.patch("/api/admin/join-applications/:id",auth,ownerOnly,async(req,res,next)=>{try{const item=await JoinApplication.findById(req.params.id);if(!item)return res.status(404).json({message:"Join application not found"});if(req.body?.status!==undefined&&!["new","reviewing","shortlisted","accepted","rejected"].includes(String(req.body.status)))return res.status(400).json({message:"Invalid application status"});if(req.body?.status!==undefined)item.status=String(req.body.status);if(req.body?.adminNote!==undefined)item.adminNote=boundedText(req.body.adminNote,2000);item.updatedAt=new Date();await item.save();res.json({application:item});}catch(e){next(e)}});
 app.delete("/api/admin/join-applications/:id",auth,ownerOnly,async(req,res,next)=>{try{const item=await JoinApplication.findByIdAndDelete(req.params.id);if(!item)return res.status(404).json({message:"Join application not found"});res.json({ok:true});}catch(e){next(e)}});
...
// Production contract markers: api/admin/upload api/notifications/subscribe api/ads/:id/impression api/ads/:id/click
